 ...
 */
export function mapFoodsDataToStoragePlaces(foodsData: FoodsLean | null, includeExpiryInfo: boolean = false): StoragePlace[] {
    ...
    return foodsData.place.map((place) => {
        ...
        const placeId = place._id instanceof Types.ObjectId ? place._id.toString() : String(place._id || '');
        ...
        return {
            ...
            foodList: place.foodList.map((food) => {
                ...
                const foodId = food._id instanceof Types.ObjectId ? food._id.toString() : String(food._id || '');
                // 유통기한 날짜 객체 생성
