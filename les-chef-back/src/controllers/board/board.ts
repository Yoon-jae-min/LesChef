...
import { PaginatedResponse, ApiSuccessResponse, ApiErrorResponse, IBoard, IBoardComment } from "../../types";
...
export const listBoards = asyncHandler(async(req: Request<{}, PaginatedResponse<IBoard> | ApiErrorResponse, {}, BoardListQuery>, res: Response<PaginatedResponse<IBoard> | ApiErrorResponse>) => {
    try {
...
export const postWriting = asyncHandler(async(req: Request<{}, ApiSuccessResponse | ApiErrorResponse, PostWritingRequestBody>, res: Response<ApiSuccessResponse | ApiErrorResponse>) => {
    if (!req.session?.user) {
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다."
...
// 게시글 상세 + 댓글
export const getBoard = asyncHandler(async(req: Request<{}, GetBoardResponse | ApiErrorResponse, {}, GetBoardQuery>, res: Response<GetBoardResponse | ApiErrorResponse>) => {
    const { id } = req.query;
    if(!id){
        res.status(400).json({
            error: true,
 ...
 * 게시글 삭제
 */
export const deleteBoard = asyncHandler(async(req: Request<{ id?: string }>, res: Response<ApiSuccessResponse | ApiErrorResponse>) => {
    if (!req.session?.user) {
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다."
...
export const editBoard = asyncHandler(async (req: Request<{ id?: string }, ApiSuccessResponse | ApiErrorResponse, EditBoardRequestBody>, res: Response<ApiSuccessResponse | ApiErrorResponse>) => {
    if (!req.session?.user) {
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다."
