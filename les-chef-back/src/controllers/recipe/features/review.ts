 ...
 * @returns 평균 평점과 리뷰 수
 */
const updateRecipeRating = async (recipeId: string | Types.ObjectId): Promise<{averageRating: number; count: number}> => {
    const allReviews = await RecipeReview.find({ recipeId }).lean();
    const count = allReviews.length;
    const averageRating =
        count > 0
            ? Math.round((allReviews.reduce((sum, r) => sum + (r.rating || 0), 0) / count) * 10) / 10 // 소수점 1자리까지
            : 0;
...
// 레시피 리뷰 목록 조회
export const listReviews = asyncHandler(async (req: Request, res: Response<ReviewListResponse | ApiErrorResponse>) => {
...
export const upsertReview = asyncHandler(async (req: Request<{}, UpsertReviewResponse | ApiErrorResponse, UpsertReviewRequestBody>, res: Response<UpsertReviewResponse | ApiErrorResponse>) => {
    if (!req.session?.user) {
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다.",
...
// 자신의 리뷰 삭제
export const deleteReview = asyncHandler(async (req: Request<{}, DeleteReviewResponse | ApiErrorResponse, DeleteReviewRequestBody>, res: Response<DeleteReviewResponse | ApiErrorResponse>) => {
    if (!req.session?.user) {
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다.",
