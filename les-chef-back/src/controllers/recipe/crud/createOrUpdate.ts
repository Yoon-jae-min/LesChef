...
export const createOrUpdate = asyncHandler(async(req: MulterRequest, res: Response<ApiSuccessResponse | ApiErrorResponse>) => {
    // 세션 검증
    ...
    try {
        const { recipeInfo, recipeIngredients, recipeSteps, isEdit, deleteImgs} = req.body as CreateOrUpdateRequestBody;
        ...
        const hasValidIngredient = parsedRecipeIngredients.some(
            (ingredient) => ingredient.ingredientUnit && ingredient.ingredientUnit.some(
        ...
        const deleteImgsArray = (Array.isArray(deleteImgs) ? deleteImgs : [deleteImgs]).filter(Boolean) as string[];
    ...
    if(userInfo.checkAdmin){
        ...
        // 레시피 대표 이미지 처리 및 썸네일 생성
        if(parsedRecipeInfo.recipeImg === ""){
            const files = req.files as { recipeImgFile?: Array<{ newPath?: string; path?: string }> } | undefined;
            if (!files?.recipeImgFile || !files.recipeImgFile[0]?.newPath) {
                res.status(400).json({
                    error: true,
        ...
        // 레시피 단계 이미지 처리 및 썸네일 생성
        const uploadedFiles = (req.files as { recipeStepImgFiles?: Array<{ newPath?: string; path?: string }> } | undefined)?.recipeStepImgFiles || []; 
        let count = 0;
        for (const step of parsedRecipeSteps) {
            if (step.stepImg === "") {
                if (uploadedFiles[count]?.newPath) {
                    ...
                    // 썸네일 생성 (비동기, 실패해도 계속 진행)
                    const stepFilePath = uploadedFiles[count].path;
                    if (stepFilePath) {
                        try {
                            const outputDir = path.dirname(stepFilePath);
                            await generateRecipeStepThumbnail(stepFilePath, outputDir);
                        } catch (thumbError) {
                            // 썸네일 생성 실패는 로그만 남기고 계속 진행
                            logger.warn(`단계 ${step.stepNum} 썸네일 생성 실패`, { error: thumbError });
                        }
                    }
                    count++;
                } else {
