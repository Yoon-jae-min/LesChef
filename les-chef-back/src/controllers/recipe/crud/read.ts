...
const getPopularityPipeline = (filter: Record<string, unknown>, skip: number, limitNum: number) => {
    return [
        { $match: filter },
        {
            $addFields: {
                popularityScore: {
                    $add: [
                        { $multiply: [{ $ifNull: ['$viewCount', 0] }, POPULARITY_WEIGHTS.VIEW_COUNT] },
                        { $multiply: [{ $ifNull: ['$averageRating', 0] }, POPULARITY_WEIGHTS.RATING] },
                        { $multiply: [{ $ifNull: ['$reviewCount', 0] }, POPULARITY_WEIGHTS.REVIEW_COUNT] }
                    ]
                }
            }
        },
    ...
    ] as any; // MongoDB aggregation pipeline 타입 호환성을 위해 any 사용
};

export const listRecipes = asyncHandler(async(req: Request<{}, PaginatedResponse<IRecipe> | ApiErrorResponse, {}, RecipeListQuery>, res: Response<PaginatedResponse<IRecipe> | ApiErrorResponse>) => {
    const {
        category = "all",
        subCategory,
        isShare,
    ...
    ) ? sort as RecipeSortOption : DEFAULT_SORT_OPTION;
    ...
    if(keyword && keyword.trim()){
        ...
        const recipeIdsFromIngredients = ingredientResults.map((item: { _id: Types.ObjectId }) => item._id);
        
        // 레시피 이름, 태그, 재료명으로 검색
        filter.$or = [
            { recipeName: keywordRegex },
    ...
    try {
        ...
        if (sortOption === RECIPE_SORT_OPTIONS.POPULAR) {
            const [totalResult, list] = await Promise.all([
                ...
                Recipe.aggregate(getPopularityPipeline(filter, skip, limitNum)).allowDiskUse(true)
            ]);
...
export const myList = asyncHandler(async(req: Request, res: Response<ApiSuccessResponse | ApiErrorResponse>) => {
    ...
    try {
        ...
        if(user.checkAdmin){
            ...
            recipeList = await Recipe.find({userId: req.session.user.id}).lean() as IRecipe[];
        }
...
export const wishList = asyncHandler(async(req: Request, res: Response<ApiSuccessResponse | ApiErrorResponse>) => {
    if(!req.session?.user?.id){
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다."
...
export const recipeInfo = asyncHandler(async(req: Request<{}, RecipeInfoResponse | ApiErrorResponse, {}, RecipeInfoQuery>, res: Response<RecipeInfoResponse | ApiErrorResponse>) => {
            ...
            if (user) {
                ...
                if(recipeWishList){
                    recipeWish = recipeWishList.wishList.some((wish) => 
                        wish.recipeId.toString() === (recipe._id as Types.ObjectId).toString()
                    );
                }
            }
        }
