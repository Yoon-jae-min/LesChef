...
import { BOARD_CATEGORY_LABEL } from "@/constants/categories";
...
export default function BoardListClient({ 
  ...
  const handleEditClick = (e: React.MouseEvent, postId: string) => {
    e.preventDefault();
    e.stopPropagation();
    router.push(`/board/edit?id=${postId}&type=${initialCategory}`);
  };

  // 게시글 목록 가져오기 - SWR 캐싱 적용
  // 서버에서 가져온 초기 데이터가 있으면 fallbackData로 사용
  const { data, error, isLoading: loading } = useSWR<BoardListResponse>(
  ...
  return (
    <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {!loading && !displayError && posts.map((post) => (
        <Link
          key={post._id}
