 ...
 * 레시피 상세 조회 (서버 컴포넌트용)
 */
export async function getRecipeDetailServer(recipeName: string) {
  try {
    const response = await serverFetch(
      `${RECIPE_API_BASE_URL}/info?recipeName=${encodeURIComponent(recipeName)}`
    );
...
export async function getBoardDetailServer(postId: string) {
  try {
    const response = await serverFetch(`${BOARD_API_BASE_URL}/${postId}`);
