 ...
 * 게시글 좋아요 토글
 */
export const toggleBoardLike = asyncHandler(async (req: Request<{}, ToggleBoardLikeResponse | ApiErrorResponse, ToggleBoardLikeRequestBody>, res: Response<ToggleBoardLikeResponse | ApiErrorResponse>) => {
    if (!req.session?.user) {
        res.status(401).json({
            error: true,
            message: "로그인이 필요합니다."
